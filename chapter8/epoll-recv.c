...
/*
無限ループ
このサンプルでは、この無限ループを抜けない
*/
    while(1){
        printf("before epoll_wait()\n");

        /*epollファイルディスクリプタに設定されたソケットが読み込み可能になるまで待つ*/
        /*ここからepoll_wait()はsock1かsock2のどちらかでデータを受信するまでブロックする*/
        ...
        for(i=0;i<nfds;i++){
            if(ev_ret[i].data.fd == sock1){//sock1側のソケットにデータを受信した場合
            ...
            }else if(ev_ret[i].data.fd == sock2){//sock2側のソケットにデータを受信した場合
