...
int my_sem_init(){
    ...
    /*
    segmet()をIPC_CREATとIPC_EXCL両方のフラグを利用して実行することによって、
    最初ではないsegmet()がEEXISTになることを期待している
    */
    ...
    if(semid >= 0){
        ...
        /*最初のsemget()が成功して、自らセマフォ識別子を作成したプロセスは、semctl()のSETALLを利用してセマフォの初期値を設定*/
        ...
        /*semop()が実行されると、最後にセマフォが利用された時間を示すsem_otimeが更新され、初期化を待つプロセスに初期化終了を知らせることになる*/
        ...
        if(errno != EEXIST){
            ...
            /*semget()がEEXISTになったプロセスでは、IPC_CREATとIPC_EXCLを利用しないsemget()によって
            既存のセマフォ識別子を得る*/
            ...
            /*semctl()のIPC_STATを利用してsem_otimeを監視しながら初期化が終了するタイミングを待つ*/
            for(;;){
